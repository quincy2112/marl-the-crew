            ...
            if np.any(eval_dones_unfinished_env):
                for idx_env in range(self.n_eval_rollout_threads):
                    if unfinished_thread[idx_env] and eval_dones_env[idx_env]:
                        ...
                        eval_win_rates[num_done] = (
                            1 if eval_infos[idx_env]["score_reward"] > 0 else 0
                        )
                        eval_steps[num_done] = (
                            eval_infos[idx_env]["max_steps"]
        ...
        for i_episode in range(self.all_args.render_episodes):
            ...
            while not np.any(render_dones):
                ...
                render_actions = np.array(
                    np.split(_t2n(render_actions), self.n_rollout_threads)
                )
                render_rnn_states = np.array(
                    np.split(_t2n(render_rnn_states), self.n_rollout_threads)
                )

                render_actions_env = [
                    render_actions[idx, :, 0] for idx in range(self.n_rollout_threads)
                ]
